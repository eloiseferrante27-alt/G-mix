...
const joueurLinks = [
  ...
  { href: '/jeu/decisions', label: 'Décisions', icon: '📋' },
  { href: '/jeu/results', label: 'Résultats', icon: '📈' },
];

export const Sidebar: React.FC<SidebarProps> = ({ role, firstName, lastName, email }) => {
  const links = role === 'joueur' ? joueurLinks : formateurLinks;
  ...
  return (
    <aside className="w-64 bg-white border-r border-slate-200 min-h-screen flex flex-col">
      ...
      <div className="p-4 border-t border-slate-200">
        ...
        <button className="mt-3 w-full flex items-center gap-2 px-3 py-2 text-sm text-slate-600 hover:bg-slate-100 rounded-lg transition-colors">
          <span>🚪</span>
